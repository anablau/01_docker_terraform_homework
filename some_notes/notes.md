...
--(to exclude data errors).
	SELECT * FROM     green_tripdata_nov
	LIMIT 10;
...
-- East Harlem North CORRECT ANSWER
-- East Harlem South
-- Morningside Heights
-- Forest Hills

	SELECT "PULocationID",b."Zone",
	       count(*) as sum_total_trips
...
-- For the passengers picked up in the zone named 
--"East Harlem North" in November 2025, 
--which was the drop off zone that had the largest tip?

-- Note: it's tip , not trip. We need the name of the zone, not the ID.

-- JFK Airport
-- Yorkville West COrrect answer
-- East Harlem North
-- LaGuardia Airport


	SELECT c."Zone" as drop_off_zone,
	...
	and lpep_pickup_datetime::date<='2025-11-30'
	group by 1
	order by 2 desc;