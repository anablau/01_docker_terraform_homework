Here is the full, cleaned-up version of your homework document. This is optimized for GitHub's Markdown renderer—I have fixed the "inline" code block errors seen in your screenshot by ensuring every command and query is on its own line with proper spacing.

You can copy and paste the entire block below into your README.md:
...
**Explanation:** Inside a Docker Compose network, containers communicate via service names. If the database service is named `db` and uses the default Postgres port `5432`, pgAdmin should connect using:
...
Answer: ```text terraform init, terraform apply -auto-approve, terraform destroy
