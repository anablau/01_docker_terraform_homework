...
FROM green_tripdata_nov
GROUP BY 1 HAVING MAX(trip_distance) <= 100 ORDER BY 2 DESC;
```
...
SQL Query: 
```
SELECT b."Zone" AS pickup_zone, SUM(total_amount) AS total_sum
FROM green_tripdata_nov a LEFT JOIN zones b ON a."PULocationID" = b."LocationID"
...
SQL Query:
```
SELECT c."Zone" AS drop_off_zone, MAX(tip_amount) AS max_tip
FROM green_tripdata_nov a LEFT JOIN zones b ON a."PULocationID" = b."LocationID"
LEFT JOIN zones c ON a."DOLocationID" = c."LocationID" WHERE b."Zone" = 'East Harlem North'
